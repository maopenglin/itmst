...
+(id)parseDic:(NSDictionary*)dic andClass:(Class)entity{
    if (!dic) {
        return nil;
    }
    id obj=[[entity alloc] init];
    unsigned int propertyCount;
    ...
    for (unsigned int i = 0; i < propertyCount; i++) {
        ...
        NSString *objvalue=[NSString stringWithFormat:@"%@",dic[@(propertyName)]];
        [self setPropertyValueForObject:propertyAttributes propertyName:@(propertyName) andObject:obj andValue:objvalue];
    }
    return obj;
}
@end
